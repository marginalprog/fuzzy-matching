    ...
    try:
        while True:
            ...
            if choice == '0':
                ...
                print(f"  {Colors.GREEN}--length-weight {Colors.CYAN}[число]{Colors.ENDC}            - вес длины строки при консолидации (по умолчанию 0.01)")
                print(f"  {Colors.GREEN}--group-fields {Colors.CYAN}[список]{Colors.ENDC}            - список дополнительных полей для группировки (например, \"Регион,Город\")")
                print(f"  {Colors.GREEN}--sort-before-match {Colors.CYAN}[t/f]{Colors.ENDC}          - сортировать записи перед сопоставлением (по умолчанию false)")
                print(f"  {Colors.GREEN}--sort-field {Colors.CYAN}[поле]{Colors.ENDC}                - поле, по которому выполнять сортировку при сопоставлении")
                print(f"  {Colors.GREEN}--transliteration-standard {Colors.CYAN}[стандарт]{Colors.ENDC} - стандарт транслитерации (GOST/Passport, по умолчанию Passport)")
